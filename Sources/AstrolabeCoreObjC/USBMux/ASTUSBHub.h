...
// ASTUSBDeviceDidAttachNotification
// Posted when a device has been attached. Also posted for each device that is
// already attached when the ASTUSBHub starts listening.
//
//  .userInfo = {
//    DeviceID = 3;
...
//  }
//
FOUNDATION_EXPORT NSString * const ASTUSBDeviceDidAttachNotification;

// ASTUSBDeviceDidDetachNotification
...
//  }
//
FOUNDATION_EXPORT NSString * const ASTUSBDeviceDidDetachNotification;

// NSError domain
FOUNDATION_EXPORT NSString * const ASTUSBHubErrorDomain;

// Error codes returned with NSError.code for NSError domain ASTUSBHubErrorDomain
typedef enum {
  ASTUSBHubErrorBadDevice = 2,
  ASTUSBHubErrorConnectionRefused = 3,
  ASTUSBHubErrorTimedOut = 1001,
} ASTUSBHubError;

@interface ASTUSBHub : NSObject
...
// callback.
//
// *timeout* limits how long the usbmuxd connection response may remain pending.
//
...
//
// *timeout* limits how long the usbmuxd listen response may remain pending.
//
