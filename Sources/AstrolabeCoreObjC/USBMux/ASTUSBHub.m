...
NSString * const ASTUSBHubErrorDomain = @"ASTUSBHubError";
...
NSString * const ASTUSBDeviceDidAttachNotification = @"ASTUSBDeviceDidAttachNotification";
NSString * const ASTUSBDeviceDidDetachNotification = @"ASTUSBDeviceDidDetachNotification";
...
// (device) system. In practice, a ASTUSBChannel is connected to a usbmuxd
// endpoint which is configured to either listen for device changes (the
// ASTUSBHub's channel is usually configured as a device notification listener) or
// configured as a TCP bridge (e.g. channels returned from ASTUSBHub's
// connectToDevice:port:callback:). You should not create channels yourself, but
// let ASTUSBHub provide you with already configured channels.
@interface ASTUSBChannel : NSObject {
...
// own I/O logic without ASTUSBChannel. Remember to dispatch_retain() the channel
// if you plan on using it as it might be released from the ASTUSBChannel at any
// point in time.
...
@interface ASTUSBChannel (Private)
...
- (void)listenWithBroadcastHandler:(void(^)(NSDictionary *packet))broadcastHandler timeout:(NSTimeInterval)timeout callback:(void(^)(NSError*))callback;
...
- (void)sendRequest:(NSDictionary*)packet timeout:(NSTimeInterval)timeout callback:(void(^)(NSError *error, NSDictionary *responsePacket))callback;
- (BOOL)completeRequestWithTag:(uint32_t)tag error:(NSError*)error responsePacket:(NSDictionary*)responsePacket;
...
@implementation ASTUSBHub


+ (ASTUSBHub*)sharedHub {
  static ASTUSBHub *gSharedHub;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    gSharedHub = [ASTUSBHub new];
    [gSharedHub listenOnQueue:dispatch_get_main_queue() timeout:kDefaultRequestTimeout onStart:^(NSError *error) {
      if (error) {
        NSLog(@"Failed: ASTUSBHub initialization failed, error=%@", error);
      }
    } onEnd:nil];
  });
  return gSharedHub;
...
- (void)listenOnQueue:(dispatch_queue_t)queue timeout:(NSTimeInterval)timeout onStart:(void(^)(NSError*))onStart onEnd:(void(^)(NSError*))onEnd {
  queue_ = queue ?: dispatch_get_main_queue();
  if (channel_) {
    if (onStart) onStart(nil);
    return;
  }
  ASTUSBChannel *channel = [ASTUSBChannel new];
  channel_ = channel;
  NSError *error = nil;
  __weak typeof(self) weakSelf = self;
  if ([channel openOnQueue:queue_ error:&error onEnd:onEnd]) {
    [channel listenWithBroadcastHandler:^(NSDictionary *packet) {
      [weakSelf handleBroadcastPacket:packet];
    } timeout:timeout callback:^(NSError *startError) {
      ASTUSBHub *strongSelf = weakSelf;
      if (startError && strongSelf && strongSelf->channel_ == channel) {
        [channel stop];
        strongSelf->channel_ = nil;
      }
      if (onStart) onStart(startError);
    }];
  } else {
    channel_ = nil;
...
- (void)connectToDevice:(NSNumber*)deviceID port:(int)port timeout:(NSTimeInterval)timeout onStart:(void(^)(NSError*, dispatch_io_t))onStart onEnd:(void(^)(NSError*))onEnd {
  ASTUSBChannel *channel = [ASTUSBChannel new];
  NSError *error = nil;
  ...
  NSDictionary *packet = [ASTUSBChannel packetDictionaryWithPacketType:kPlistPacketTypeConnect
                                                             payload:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                      deviceID, @"DeviceID",
                                                                      [NSNumber numberWithInt:port], @"PortNumber",
                                                                      nil]];

  [channel sendRequest:packet timeout:timeout callback:^(NSError *error_, NSDictionary *responsePacket) {
...
- (void)handleBroadcastPacket:(NSDictionary*)packet {
  ...
  if ([@"Attached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ASTUSBDeviceDidAttachNotification object:self userInfo:packet];
  } else if ([@"Detached" isEqualToString:messageType]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:ASTUSBDeviceDidDetachNotification object:self userInfo:packet];
  } else {
    NSLog(@"Failed: received an unsupported USB broadcast message, packet=%@", packet);
  }
}
...
- (void)listenWithBroadcastHandler:(void(^)(NSDictionary *packet))broadcastHandler timeout:(NSTimeInterval)timeout callback:(void(^)(NSError*))callback {
  ...
  NSDictionary *packet = [ASTUSBChannel packetDictionaryWithPacketType:kPlistPacketTypeListen payload:nil];

  [self sendRequest:packet timeout:timeout callback:^(NSError *error_, NSDictionary *responsePacket) {
    if (!callback)
      return;
...
- (BOOL)errorFromPlistResponse:(NSDictionary*)packet error:(NSError**)error {
  if (!*error) {
    ...
    if (!n) {
      *error = [NSError errorWithDomain:ASTUSBHubErrorDomain code:(n ? n.integerValue : 0) userInfo:nil];
      return NO;
    }
    ...
    if (replyCode != 0) {
      ...
      *error = [NSError errorWithDomain:ASTUSBHubErrorDomain code:replyCode userInfo:[NSDictionary dictionaryWithObject:errmessage forKey:NSLocalizedDescriptionKey]];
      return NO;
    }
  }
  return YES;
...
- (void)sendRequest:(NSDictionary*)packet timeout:(NSTimeInterval)timeout callback:(void(^)(NSError*, NSDictionary*))callback {
  ...
  [responseQueue_ setObject:callback forKey:key];
  ...
  dispatch_after(
    ...
    queue_,
    ^{
      ASTUSBChannel *strongSelf = weakSelf;
      if (!strongSelf) {
        return;
      }
      NSError *error = [NSError errorWithDomain:ASTUSBHubErrorDomain
                                           code:ASTUSBHubErrorTimedOut
                                       userInfo:@{NSLocalizedDescriptionKey: @"Timed out while waiting for a usbmuxd response"}];
      [strongSelf completeRequestWithTag:tag error:error responsePacket:nil];
    }
  );

  [self sendPacket:packet tag:tag callback:^(NSError *error) {
    if (error) {
      [self completeRequestWithTag:tag error:error responsePacket:nil];
    }
  }];
...
- (BOOL)completeRequestWithTag:(uint32_t)tag error:(NSError*)error responsePacket:(NSDictionary*)responsePacket {
  ...
  requestCallback(error, responsePacket);
  return YES;
}
...
- (void)scheduleReadPacketWithBroadcastHandler:(void(^)(NSDictionary *packet))broadcastHandler {
  ...
  [self scheduleReadPacketWithCallback:^(NSError *error, NSDictionary *packet, uint32_t packetTag) {
    ...
    if (packetTag == 0) {
      ...
      // Reply
      if (![self completeRequestWithTag:packetTag error:error responsePacket:packet]) {
        NSLog(@"Failed: ignored a usbmuxd response without a matching callback, packet=%@", packet);
      }
    }
...
- (void)scheduleReadPacketWithCallback:(void(^)(NSError*, NSDictionary*, uint32_t))callback {
  ...
  dispatch_io_read(channel_, 0, sizeof(ref_upacket.size), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    AST_PRECISE_LIFETIME_UNUSED dispatch_data_t map_data = dispatch_data_create_map(data, (const void **)&buffer, &buffer_size); // objc_precise_lifetime guarantees 'map_data' isn't released before memcpy has a chance to do its thing
    ...
    off_t offset = sizeof(ref_upacket.size);
      ...
      if (upacket_len > kUsbmuxPacketMaxPayloadSize) {
        callback(
          [[NSError alloc] initWithDomain:ASTUSBHubErrorDomain code:1 userInfo:@{
      ...
      char *buffer = NULL;
      size_t buffer_size = 0;
      AST_PRECISE_LIFETIME_UNUSED dispatch_data_t map_data = dispatch_data_create_map(data, (const void **)&buffer, &buffer_size);
      ...
      if (upacket->protocol != USBMuxPacketProtocolPlist) {
        callback([[NSError alloc] initWithDomain:ASTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package protocol" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
      ...
      if (upacket->type != USBMuxPacketTypePlistPayload) {
        callback([[NSError alloc] initWithDomain:ASTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package type" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
